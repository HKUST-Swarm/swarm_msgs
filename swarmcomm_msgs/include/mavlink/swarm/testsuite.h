...
static void mavlink_test_NodeDetected(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_NodeDetected >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_NodeDetected_t packet_in = {
        93372036854775807LL,963497880,963498088,129.0,157.0,185.0,213.0,241.0,269.0,297.0,325.0
    };
    mavlink_NodeDetected_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        ...
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_NodeDetected_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_NodeDetected_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_NodeDetected_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_NodeDetected_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_NodeDetected_pack(system_id, component_id, &msg , packet1.lps_time , packet1.id , packet1.target_id , packet1.rel_x , packet1.rel_y , packet1.rel_z , packet1.rel_yaw , packet1.cov_x , packet1.cov_y , packet1.cov_z , packet1.cov_yaw );
    mavlink_msg_NodeDetected_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_NodeDetected_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.lps_time , packet1.id , packet1.target_id , packet1.rel_x , packet1.rel_y , packet1.rel_z , packet1.rel_yaw , packet1.cov_x , packet1.cov_y , packet1.cov_z , packet1.cov_yaw );
    mavlink_msg_NodeDetected_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_NodeDetected_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_NodeDetected_send(MAVLINK_COMM_1 , packet1.lps_time , packet1.id , packet1.target_id , packet1.rel_x , packet1.rel_y , packet1.rel_z , packet1.rel_yaw , packet1.cov_x , packet1.cov_y , packet1.cov_z , packet1.cov_yaw );
    mavlink_msg_NodeDetected_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_swarm(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_NodeDetected(system_id, component_id, last_msg);
