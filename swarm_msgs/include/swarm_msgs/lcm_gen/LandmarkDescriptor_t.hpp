...
int LandmarkDescriptor_t::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = this->landmark._encodeNoHash(buf, offset + pos, maxlen - pos);
    if(tlen < 0) return tlen; else pos += tlen;
...
int LandmarkDescriptor_t::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = this->landmark._decodeNoHash(buf, offset + pos, maxlen - pos);
    if(tlen < 0) return tlen; else pos += tlen;
...
{
    int enc_size = 0;
    enc_size += this->landmark._getEncodedSizeNoHash();
    enc_size += __int32_t_encoded_array_size(NULL, 1);
    enc_size += __float_encoded_array_size(NULL, this->desc_len);
    enc_size += __int32_t_encoded_array_size(NULL, 1);
...
uint64_t LandmarkDescriptor_t::_computeHash(const __lcm_hash_ptr *p)
{
    ...
    uint64_t hash = 0xd21abf75eaa17cdaLL +
