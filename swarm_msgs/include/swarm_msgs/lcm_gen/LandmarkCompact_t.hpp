...
#ifndef __LandmarkCompact_t_hpp__
#define __LandmarkCompact_t_hpp__

#include <lcm/lcm_coretypes.h>

#include "Time_t.hpp"
#include "Point2f_t.hpp"
#include "Point3f_t.hpp"
#include "Point3f_t.hpp"
#include "Point3f_t.hpp"


class LandmarkCompact_t
{
    public:
        int32_t    landmark_id;
...
int LandmarkCompact_t::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int LandmarkCompact_t::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;
...
int LandmarkCompact_t::getEncodedSize() const
{
    return 8 + _getEncodedSizeNoHash();
}

int64_t LandmarkCompact_t::getHash()
{
    static int64_t hash = static_cast<int64_t>(_computeHash(NULL));
    return hash;
}

const char* LandmarkCompact_t::getTypeName()
{
    return "LandmarkCompact_t";
}

int LandmarkCompact_t::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
    ...
    tlen = this->stamp_discover._encodeNoHash(buf, offset + pos, maxlen - pos);
    if(tlen < 0) return tlen; else pos += tlen;
    ...
    tlen = __float_encode_array(buf, offset + pos, maxlen - pos, &this->depth, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    return pos;
}

int LandmarkCompact_t::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;
    ...
    tlen = this->stamp_discover._decodeNoHash(buf, offset + pos, maxlen - pos);
    if(tlen < 0) return tlen; else pos += tlen;
    ...
    tlen = __float_decode_array(buf, offset + pos, maxlen - pos, &this->depth, 1);
    if(tlen < 0) return tlen; else pos += tlen;

    return pos;
}

int LandmarkCompact_t::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
    enc_size += __int32_t_encoded_array_size(NULL, 1);
    enc_size += this->stamp_discover._getEncodedSizeNoHash();
...
uint64_t LandmarkCompact_t::_computeHash(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for(fp = p; fp != NULL; fp = fp->parent)
        if(fp->v == LandmarkCompact_t::getHash)
            return 0;
    const __lcm_hash_ptr cp = { p, LandmarkCompact_t::getHash };

    uint64_t hash = 0x8ee86c833f01b376LL +
         Time_t::_computeHash(&cp) +
         Point2f_t::_computeHash(&cp) +
